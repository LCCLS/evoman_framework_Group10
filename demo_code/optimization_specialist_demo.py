...
####   Optimization for cont
# roller solution (best genotype-weights for phenotype-network): Ganetic Algorihm    ###
...
# normalizes
def norm(x, pfit_pop):
    if (max(pfit_pop) - min(pfit_pop)) > 0: # if the result of min max norm is bigger than 0, return result
        x_norm = (x - min(pfit_pop)) / (max(pfit_pop) - min(pfit_pop))
    else: # if the result is negative, return 0
        x_norm = 0
...
for i in range(ini_g + 1, gens):
    ...
    fit_pop_norm = np.array(list(map(lambda y: norm(y, fit_pop_cp),
                                     fit_pop)))  # avoiding negative probabilities, as fitness is ranges from
    # negative numbers
